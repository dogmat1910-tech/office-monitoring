"""
Агрегированный счётчик клавиш по активному приложению.

⚠️ НЕ хранит и НЕ отправляет содержимое нажатий — только число нажатий per
(app_name, domain). Это даёт метрику «насколько активно менеджер пишет
в каждом окне» без юр.рисков полного кейлоггера.

На macOS требует Accessibility-разрешения (как и трекинг окон).
...
class KeystrokeAggregator:
    """
    get_window_fn() возвращает {"app_name": ..., "title": ...} (что показывает active_window).
    domain (если нужен) извлекается на сервере из title.
    """

    def __init__(self, get_window_fn) -> None:
        self.get_window_fn = get_window_fn
        self._lock = threading.Lock()
        self._counts: dict[str, int] = defaultdict(int)  # ключ: app_name|domain
        self._listener = None
    ...
    def start(self) -> bool:
        ...
        def _on_press(key):  # noqa: ANN001
            try:
                ...
                # для браузеров — пытаемся вытащить домен из title формата "page — https://..."
    ...
    def drain(self) -> list[dict]:
        """Забирает накопленные счётчики и сбрасывает буфер.
        Возвращает список dict-ов готовых к отправке."""
        with self._lock:
